...
// 基金公司直销渠道（App）限额覆写。
// 大部分基金的限额与天天基金/东方财富接口返回的 maxBuy 一致，
// 此处仅收录已知直销限额 ≠ 第三方平台限额的少数代码。
// 格式: { "000000": "500000" } 单位为元。
// 注意：此覆写在实时数据可用时生效，fallback 模式不适用。
// 天天基金/东方财富接口未返回限额数据的特殊份额（I/F/D类等），
// 手动从各基金公司公告补充。数据日期标注在注释中。
